-- ============================================================
-- cf-vps-monitor 全量安全升级 SQL（v2.0.3）
-- 由官方迁移 1~5 号拼接，全部幂等：表/列/索引/函数已存在则跳过
-- 可直接在 Supabase SQL Editor 一次性执行，重复执行也不报错
-- 建议先备份：Supabase 控制台 -> Backups
-- ============================================================

-- ################################################################
-- # 迁移文件: 1_core_schema.sql
-- ################################################################
...
-- Each observed authentication state has an identity, including after delete/recreate.
alter table public.login_rate_limits
  add column if not exists failure_revision uuid not null default gen_random_uuid();
...
-- New deliveries carry an idempotency hash. Existing history is left untouched.
alter table public.ping_snapshots add column if not exists batch_hash text;
create unique index if not exists idx_ping_snapshots_delivery
  on public.ping_snapshots(client, time, batch_hash) where batch_hash is not null;

create table if not exists website_monitors (
  id bigint generated by default as identity primary key,
  config_revision uuid not null default gen_random_uuid(),
  name text not null,
  url text not null,
...
begin
  -- public is shared with other applications. Revoke only this application's
  -- objects; schema access and owner defaults belong to the project owner.
  foreach role_name in array array['public', 'anon', 'authenticated'] loop
    if role_name = 'public' or to_regrole(role_name) is not null then
      foreach table_name in array app_tables loop
        execute format('revoke all on table public.%I from %s', table_name, role_name);
      end loop;
      for sequence_oid in
      ...
      loop
        execute format('revoke all on sequence %s from %s', sequence_oid::regclass, role_name);
      end loop;
    end if;
  end loop;
end $$;
...
set search_path = public
as $$
  -- 安全约束：本函数向所有调用方（含游客路径）开放，SELECT 字段即对外契约。
  -- ipv4/ipv6 永远不允许出现在公开接口（防止泄露节点真实出口 IP）。
  -- tags 在数据库层就剔除 IP 类标签（worker 还有第二层过滤，纵深防御）。
  -- region 用 has_ipv4/has_ipv6 的布尔标记替代具体地址（前端展示用）。
  select coalesce(jsonb_agg(to_jsonb(row_data)), '[]'::jsonb)
  from (
    select
      uuid, name, cpu_name, virtualization, arch, cpu_cores, os,
      kernel_version, gpu_name,
      (ipv4 is not null and ipv4 <> '' and ipv4 !~ '^(10\.|192\.168\.|172\.(1[6-9]|2[0-9]|3[01])\.|169\.254\.|127\.|0\.|100\.64\.|::1$|fe80:|fc00:|fd)')::boolean as has_ipv4,
      (ipv6 is not null and ipv6 <> '' and ipv6 !~ '^(10\.|192\.168\.|172\.(1[6-9]|2[0-9]|3[01])\.|169\.254\.|127\.|0\.|100\.64\.|::1$|fe80:|fc00:|fd)')::boolean as has_ipv6,
      region, public_remark,
      mem_total, swap_total, disk_total, version, price, billing_cycle,
      auto_renewal, currency, expired_at, "group",
      coalesce((
        select string_agg(trim(tag), ';')
        from unnest(string_to_array(tags, ';')) as tag
        where lower(trim(regexp_replace(tag, '<\w+>$', ''))) not in ('ipv4','ipv6','ip4','ip6','v4','v6')
          and trim(tag) <> ''
      ), '')::text as tags,
      hidden,
